...
def get_free_parking_lots(prediction) -> int:
    ...
    for i in range(len(prediction)):
        ...
        if area > width * height:
            x0, x1, y0, y1 = int(prediction[i - 1][0]), int(obj[0]), int(prediction[i - 1][1]), int(obj[1])
        else:
            x0, x1, y0, y1 = int(obj[0]), int(prediction[i - 1][0]), int(obj[1]), int(prediction[i - 1][1])
...
def predict_image(image_path: str, camera_position: int):
    # opening picture and bitmap
    image_0 = Image.open(image_path)
    bitmap = Image.open(f"pictures/bitmap_parkinglot_{camera_position}.png")
    ...
    # modify image_path and save the new image there
    image_path = image_path.replace('.png', '')
    new_im.save(f"{image_path}_edit.png")

    # get prediction from yolov5
    results = yolov5.predict(f"{image_path}_edit.png")
...
if __name__ == "__main__":
    print(get_free_parking_lots(predict_image('./pictures/parkinglot_3.png', 3)))
